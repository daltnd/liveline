...
export function createLivelineEngine(
  ...
  const st = {
    ...
    gridState: { interval: 0, labels: new Map<number, number>() }, // labels: key=Math.round(val*1000), value=alpha
    ...
    badgeColor: { green: 1 },
    badgeY: null as number | null, // lerped badge Y, null = uninited
    reducedMotion: false,
    size: { w: 0, h: 0 },
    ...
    scrubAmount: 0, // 0 = not scrubbing, 1 = fully scrubbing
    ...
    // Pause state
    pauseProgress: 0, // 0 = playing, 1 = fully paused
    timeDebt: 0, // accumulated seconds behind real time
    ...
    closeLineSmooth: 0,         // smooth close for dashed line — never resets on candle birth
